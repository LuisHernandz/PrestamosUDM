...
@section('titulo', 'UDM - Bibliotecarios')
            ...
            <div class="table-wrapper">
                <div class="table-title">
                    <div class="row">
                        ...
                        <div class="" style="width: 50%;">
                            <a href="{{ route('/admin/usuarios/bibliotecarios.create') }}" class="btn btn-add" >
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$usuario -> bib_nombre}} {{$usuario -> bib_apellidos}}</td>
                            <td>{{$usuario -> email}}</td>
                            ...
                            <td>
                                ...
                                <a href="{{ url('admin/usuarios/bibliotecarios/modificar/'.$usuario -> id) }}" class="edit">
                ...
                <div class="clearfix">
                    ...
                    <ul class="pagination">
                        ...
                        @else
                            <li class="page-item">
                                <a class="page-link page-next" href="{{ !$usuarios->onFirstPage() ? $usuarios->appends([])->previousPageUrl() : '#' }}">Anterior</a>
                            </li>
                        @endif
                        ...
                        @if ($usuarios->lastPage() <= 5)
                            @foreach ($usuarios->getUrlRange(1, $usuarios->lastPage()) as $page => $url) 
                                <li class="page-item {{ $page == $usuarios->currentPage() ? 'active' : '' }}">
                                    <a href="{{ $usuarios->appends([])->url($page) }}" class="page-link">{{ $page }}</a>
                                </li>
                            @endforeach
                        @else
            ...
            <div class="modal-content" >
                <form>
                    <div class="modal-header">						
                        <h4 class="modal-title">Datos Del Bibliotecario:</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    </div>
                    <div class="modal-body">					

                        <div class = "">
                            <p><span>Nombre:</span> {{$usuario -> bib_nombre}} {{$usuario -> bib_apellidos}}</p>
                        </div>
                        ...
                        <div class = "">
                            <p><span>No. Teléfono:</span> {{$usuario -> telefono}}</p>
                        </div>
                        
                        <div class = "">
                            <p><span>Domicilio:</span> {{$usuario -> domicilio}}</p>
                        </div>

                        <div class = "">
                            <p><span>Correo electrónico:</span> {{$usuario -> email}}</p>
                        </div>
...
@foreach ($usuarios as $usuario)
    <div id="delete-{{$usuario -> id}}" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ url('admin/usuarios/bibliotecarios/eliminar/'.$usuario -> id) }}" method = "post">
                    @csrf
                    {{ method_field ('DELETE') }}
                    <div class="modal-header">						
                        <h4 class="modal-title">Eliminar Bibliotecario</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    </div>
                    <div class="modal-body">					
